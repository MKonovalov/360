...
// NousResearch-only settings (nousresearch/hermes-4-70b, no fallbacks), and
...
// (src/lib/agents/nousresearch-only-chain.test.ts) to assert — never env
// values, never key material (Security Domain / T-27-01).
//
...
async function main() {
  ...
  // NousResearch-only chain — no other provider id anywhere in the chain (the
