...
import { noStoreJson, parsePositiveLocalId, safeSearchReviewProjection } from '@/lib/search/routeSupport';

export const dynamic = 'force-dynamic';
export const revalidate = 0;

export async function GET(_request: Request, context: RouteContext<'/api/search-runs/[id]/reviews'>): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof Error) return noStoreJson({ error: 'search_reviews_unavailable' }, 503);
    throw error;
  }
  switch (reconciliation.kind) {
    ...
    case 'processing_failed':
      return noStoreJson({ error: 'search_reviews_unavailable' }, 503);
  ...
  try {
    const reviews = await listSearchReviews(searchRunId, userId);
    return noStoreJson(reviews.map(safeSearchReviewProjection));
  } catch (error: unknown) {
    if (error instanceof Error) return noStoreJson({ error: 'search_reviews_unavailable' }, 503);
    throw error;
  }
