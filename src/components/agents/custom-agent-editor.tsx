...
export function CustomAgentEditor({ mode, practiceAreas, capabilities, agent, issues = [], trigger, open, onOpenChange }: EditorProps) {
  ...
  const outputSchemaIssue = validationIssues.find(
    (issue) => issue.path === 'outputSchema' || issue.path.startsWith('outputSchema.'),
  );
      ...
      <SheetContent side="right" className="w-full overflow-hidden sm:max-w-2xl" aria-label={mode === 'create' ? 'Create custom agent' : `Edit ${name}`}>
        ...
        <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
          <div className="flex-1 space-y-5 overflow-y-auto px-4 pb-4">
            <section className="flex flex-col gap-3" aria-labelledby="custom-identity-heading"><h2 id="custom-identity-heading" className="text-sm font-semibold text-slate-900">Identity</h2><Input aria-label="Name" aria-invalid={fieldError('name') ? true : undefined} value={name} onChange={(event) => setName(event.target.value)} placeholder="Agent name" /><Textarea aria-label="Description" aria-invalid={fieldError('description') ? true : undefined} value={description} onChange={(event) => setDescription(event.target.value)} placeholder="What this agent is for" rows={3} />{fieldError('name') ? <p role="alert" className="text-xs text-red-600">{fieldError('name')?.message}</p> : null}{fieldError('description') ? <p role="alert" className="text-xs text-red-600">{fieldError('description')?.message}</p> : null}</section>
            ...
            <section className={sectionClass} aria-labelledby="custom-query-heading"><h2 id="custom-query-heading" className="text-sm font-semibold text-slate-900">Query / Behavior</h2><label className="flex flex-col gap-1 text-xs font-medium text-slate-700">Research query / objective<Textarea aria-label="Research query" aria-invalid={fieldError('researchQuery') ? true : undefined} value={researchQuery} onChange={(event) => setResearchQuery(event.target.value)} rows={4} /></label>{fieldError('researchQuery') ? <p role="alert" className="text-xs text-red-600">{fieldError('researchQuery')?.message}</p> : null}<label className="flex flex-col gap-1 text-xs font-medium text-slate-700">Behavior instruction<Textarea aria-label="Behavior instruction" aria-invalid={fieldError('behaviorInstruction') ? true : undefined} value={behaviorInstruction} onChange={(event) => setBehaviorInstruction(event.target.value)} rows={4} /></label>{fieldError('behaviorInstruction') ? <p role="alert" className="text-xs text-red-600">{fieldError('behaviorInstruction')?.message}</p> : null}</section>
