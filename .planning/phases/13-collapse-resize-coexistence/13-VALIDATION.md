...
| 13-02-01 | 02 | 2 | COLR-01..03 (regression) | T-13-01..04 | Full phase regression: rail activates end-to-end (all dormant classes live), ⌘B + sidebar_state cookie + drag-resize byte-identical, sweep + fence + full suite + build green | sweep + fence + unit | `test -z "$(grep -rnE 'indigo|amber|#[0-9a-fA-F]{3,8}|\bdark:' src/components/layout/)"` → `sweep-clean`; `git diff <base> HEAD -- src/components/ui/sidebar.tsx src/components/ui/tooltip.tsx src/components/ui/dropdown-menu.tsx src/app/globals.css src/app/(dashboard)/layout.tsx src/components/layout/app-shell-layout.tsx package.json package-lock.json` = empty; `npm test`; `npx vitest run src/lib/nav.test.ts --bail=1` (11/11); `npm run build` | nav.test.ts ✅ (Phase 10) | ⬜ pending |
