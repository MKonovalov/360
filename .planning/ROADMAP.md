...
| 23. Provider Registry + Servable Sources | v1.5 | 0/0 | Not started | - |
...
**Success Criteria** (what must be TRUE):
  ...
  5. `PROVIDER_DEFAULT_MODELS` gains nousresearch + opencode reset targets; `NOUSRESEARCH_API_KEY` + `OPENCODE_API_KEY` are declared optional server-only (`env.ts`, `.env.example`, Vercel env); cross-provider chains spanning the new providers pass union-wide save validation.
**Plans**: TBD
