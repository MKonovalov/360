...
**Milestone v1.3 (AI Model Settings) in progress — Phase 17 complete 2026-08-02.** 3/3 plans executed (Settings UI + List Source), SET-01..07 validated 5/5 must-haves (goal-backward verification against actual code). Staff can now open Settings from the shared nav (sidebar Manage group + both ExplorerMenus), see their saved primary + ordered fallback chain (clear empty state), pick only from the runnable allowlist ∩ committed snapshot (`claude-sonnet-4-6`), add/remove/reorder up to 2 fallbacks, and save via the `saveSettingsAction` Server Action (session-only userId, zod-validated, dedupe backstop, atomic upsert, `revalidatePath` on success only). D-01 roster re-verified live (2026-08-02): undated haiku-4-5 still absent → allowlist stays sonnet-only. 288 tests green, tsc clean, build exit 0 incl. `/settings` route. Code review: 0 critical / 2 advisory warnings (form save-rejection catch, stale "Saved." label). Interactive form UAT deferred to Phase 18 VER-03. Next: Phase 18 Verification Gate (VER-01..04).
