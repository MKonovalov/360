...
**Milestone v1.3 (AI Model Settings) shipped 2026-08-02.** All 4 phases complete (12 plans). Phase 18 (Verification Gate) closed the milestone: VER-01 failover taxonomy proven by 4 new loop-level Vitest tests (401/403/output-schema never advance, RetryError-404 unwraps and advances) + existing loop/classify matrix; VER-02 catalog/chain cells locked with a real-snapshot test (committed 1131-model catalog → exactly `['claude-sonnet-4-6']`, zero `opencode/`/dated leakage) and an explicit partial-chain resolve test; VER-03 live-browser UAT proved Settings → Analyze → `agent_run.model_used` equals the saved primary (actual Postgres row id=3, `model_used: claude-sonnet-4-6`, `model_chain: [claude-sonnet-4-6]`, verified byte-for-byte by the verifier); VER-04 deployed preview (PR #1, Vercel auto-preview + full CLI redeploy) renders `/settings` from the committed catalog with no 500/empty/leakage, anonymous visits Clerk-gated, zero-hit `exec|spawn|child_process` grep in `src/`. All 13 "Looks Done But Isn't" checklist items dispositioned in `18-VER-01-MATRIX.md`; SC-3 forced-fail clause satisfied-by-extension via the Vitest loop tests. 294 tests green, tsc clean, code review 0 critical / 2 advisory (test-quality notes). Full milestone delivered: per-user model chain (primary + up to 2 fallbacks) persisted via atomic upsert, servable-allowlist pickers from the committed catalog snapshot, error-driven failover orchestration with `model_used`/`model_chain` audit, and the verification gate proving every requirement cell.
