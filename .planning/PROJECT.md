...
**Milestone v1.3 (AI Model Settings) in progress — Phase 16 complete 2026-08-02.** 4/4 plans executed (Failover Orchestration), FAL-01..05 validated 8/8 must-haves (goal-backward verification against actual code). The Analytic Agent now consumes each user's saved model chain — resolved once at run start (snapshot-at-entry) — and retries down it on provider/model failures within the 60s Vercel ceiling, failing loud (never a silent switch) when the chain is exhausted or the error is not model-related. Failover foundation: `classifyModelError` (RetryError-unwrap-first, exact `{model_not_found, server_error, connection}` eligibility set — 429/4xx/output/config never advance), `resolveModelChain` (dedupe → allowlist → cap-2), bounded chain loop with per-attempt `{ totalMs }` budgets clamped to the 54s loop wall (WR-03 closure), and `modelUsed`/`model_chain` audit identity persisted via `createRun` + surfaced in the Analyze status strip (D-04 `rate_limited` copy, D-06 fallback note). UAT found two regressions fixed in-phase (35s budget aborted real 43-50s tool-loop analyses; result spread dropped ai@7 prototype getters) — 277 tests green. Two human UAT items tracked partial (live-browser strip rendering, live-run audit trail). Next: Phase 17 Settings UI + List Source (SET-01..07).
...
---
*Last updated: 2026-08-02 — v1.3 Phase 16 (Failover Orchestration) complete*
